...
def get_pbp(link):
    ...
    for i in pbp:
        ...
        i['game_id'] = game_id
        i['posteam'] = f"{team0['location']} {team0['nickname']}" if i['startPossession']['teamId'] == team0['teamId'] else f"{team1['location']} {team1['nickname']}"
        i['defteam'] = f"{team0['location']} {team0['nickname']}" if i['startPossession']['teamId'] == team1['teamId'] else f"{team1['location']} {team1['nickname']}"
        ...
        if 'No Play' in i['playText']:
            ...
            if i['playType']['name'] == 'Rush':
                ...
                if rusher:
                    ...
                    i['rush_yards'] = int(re.findall('-?\d+\syds', i['playText'])[0].replace(' yds',''))
                ...
                if 'Incomplete' in i['playText']:
                    i['pass_attempt'] = 1
                    i['passer'] = re.findall('[A-Z]\.\s.+\sIncomplete', i['playText'])[0].replace(' Incomplete','')
                    i['pass_yds'] = 0
                    receiver = re.findall('for\s[A-Z]\.\s.+\sat', i['playText'])
                    if receiver:
                        i['receiver'] = receiver[0].replace('for ','').replace(' at','')
                elif 'Complete' in i['playText']:
                    i['pass_attempt'] = 1
                    i['passer'] = re.findall('[A-Z]\.\s.+\sComplete', i['playText'])[0].replace(' Complete','')
                    i['pass_yds'] = int(re.findall('-?\d+\syds', i['playText'])[0].replace(' yds', ''))
                    yac = re.findall('-?\d+\sYAC', i['playText'])
                    i['yac'] = int(yac[0].replace(' YAC', '')) if yac else np.nan
                    i['receiver'] = re.findall('to\s[A-Z]\.\s.+,\scaught', i['playText'])[0].replace('to ','').replace(', caught','')
                    i['pass_td'] = 1 if 'Touchdown' in i['playText'] else 0
                elif 'Intercept' in i['playText']:
                    i['pass_attempt'] = 1
                    i['passer'] = re.findall('[A-Z]\.\s.+\sIntercepted', i['playText'])[0].replace(' Intercepted','')
                    i['pass_yds'] = 0
                    i['interception'] = 1
                ...
                if returner:
                    i['kick_returner'] = returner[0].replace('by ','').replace(' from','')
                    
                    i['kick_return_yds'] = int(re.findall('-?\d+\syds', i['playText'])[1].replace(' yds', ''))
                ...
                returner = re.findall('by\s[A-Z]\.\s.+\sfrom', i['playText'])
                if returner:
                    i['punt_returner'] = returner[0].replace('by ','').replace(' from','')
                    
                    i['punt_return_yds'] = int(re.findall('-?\d+\syds', i['playText'])[1].replace(' yds', ''))
